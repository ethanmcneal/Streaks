        ...
        <Card className="timer" >
         ...
         {winner ? <div><div style={{textAlign: 'center', padding:'10em'}}>
          ...
          {/* <ListGroupItem>{streak.open == true ? 'joinable' : 'private'}</ListGroupItem> */}
            ...
            {user.id === streak.owner && <Button variant="warning" className='button-orange' onClick={deleteStreak}>Delete Streak</Button>}
            {user.id === streak.owner && <Link  to={`/streaks/edit/${id}`}><Button variant="warning" className='button-orange'>Edit Streak</Button></Link>}
            {user.id === streak.owner && streak.open == false ? <Button variant="warning" className='button-orange'  onClick={()=>closeStreak(true)}>Make Streak Public</Button> : 
            user.id === streak.owner ? <Button variant="warning" className='button-orange' onClick={()=>closeStreak(false)}>Make Streak Private</Button> : '' }
            </div>
