...
const Streak = () => {
  ...
  const getStreak = async () => {
    try {
      ...
      setStreak({
        name: res.data[0].streak_name,
        description: res.data[0].description,
        reward: res.data[0].reward,
        punishment: res.data[0].punishment,
        timeline: res.data[0].timeline,
        owner: res.data[0].owner,
        open: res.data[0].public,
        streak_id: res.data[0].streak_id,
      });
      setUsers(res.data);
  ...
  const setChampion = async (champion) => {
    try {
      setWinner(champion);
      let res = await axios.patch(`/api/user_streaks/${champion.id}`, {
        status: "won",
      });
  ...
  const winnerCheck = (participants) => {
    participants.map((user) =>
      user.status == "ongoing" || user.status == "won"
      ...
      <div>
        <div style={{ display: "flex" }}>
          ...
          <div>
            ...
            <p
              style={{ color: "black", textAlign: "left", marginLeft: "1.5em" }}
            >
              {streak.description}
            </p>
            <div style={{ display: "flex" }}>
              <ListGroupItem style={{ color: "rgb(141 129 140)" }}>
                Created by:
              </ListGroupItem>
              <ListGroupItem style={{ color: "rgb(43 174 173)" }}>
                {creator.nickname}
              </ListGroupItem>
            </div>
          </div>
        ...
        <div>
          <div style={{ display: "flex" }}>
            <img
              src={trophy}
              style={{ width: "25px", height: "25px", marginLeft: "5em" }}
            />
    ...
    <>
      ...
      <div>
        <Card
          ...
          style={{ marginLeft: "10em", borderRadius: "3px" }}
        >
          <h3>Participants</h3>
          ...
          <Container className="mt-5 mb-5 justify-content-center">
            <Card className="timer">
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                ...
                {user.id === streak.owner && (
                  <Link to={`/streaks/edit/${id}`}>
                    <Button variant="warning" className="button-orange">
                      Edit Streak
                    </Button>
                  </Link>
                )}
                {user.id === streak.owner && streak.open == false ? (
                  <Button
                    variant="warning"
                    className="button-orange"
                    onClick={() => closeStreak(true)}
                  >
                    Make Streak Public
                  </Button>
                ) : user.id === streak.owner ? (
