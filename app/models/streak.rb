class Streak < ApplicationRecord
    ...
    def self.streak_all_comments(streak_id)
        select('c.info, c.media, c.cheer, c.laugh, c.id as comment_id, u.name as user_name, u.nickname, u.image, u.email, u.wins, u.losses, u.id as user_id, s.name as streak_name, s.timeline, s.description, s.category, s.reward, s.open as pub, s.punishment, s.id as streak_id') 
        .from('comments as c')
